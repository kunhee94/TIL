...
for _ in range(t):     #단어갯수만큼 단어 받아야함
    ...
    for i in range(len(word)):      #단어를 인덱스로 돌자
        if i==0:										  #첫번째 알파벳은 비교대상없으니까 더해줌
            re_word+=word[i]
        elif word[i] in re_word and word[i]==word[i-1]:   #이미 문자열에 있지만 연속일경우는 더해줘야됨
            re_word+=word[i]							  
        elif word[i] in re_word and word[i]!=word[i-1]:   #이미 문자열에 있고 연속이 아니면 더하면 안됨
            continue
        else:
